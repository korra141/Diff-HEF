...
from src.utils.metrics import kl_divergence_s1,root_mean_square_error_s1,mean_absolute_error, expected_calibration_error_continuous, compute_cdf_from_pdf, predicted_residual_error, wasserstein_distance,expected_calibration_error, sharpness_discrete, expected_calibration_error_check
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
      ...
      for i, (ground_truth, measurements) in enumerate(train_loader):
        ...
        # Compute the loss
          ...
          if args.range_theta == None:
            ...
            true_density = true_distribution.density_local(args.range_theta)
          ece = expected_calibration_error(predicted_density, true_density, M=10)
          ece_check = expected_calibration_error_check(predicted_density, true_density, M=10)
          ece_tot += ece
          ece_tot_check += ece_check
          sharpness += sharpness_discrete(predicted_density)
          true_nll += true_distribution.negative_loglikelihood(measurements.numpy())
          kl_div_tot += kl_divergence_s1(true_density, predicted_density)
          wd_tot += wasserstein_distance(predicted_density, true_density)
          if args.wandb and epoch % 10 == 0 and i == sample_batch:
            indices = np.random.choice(args.batch_size, 5, replace=False)
            for j in indices:
              fig, ax = plt.subplots()
              if args.range_theta == None and args.gaussian_parameterisation:
      ...
      print(f"ECE Check: {ece_tot_check/len(train_loader):.4f}")
      metrics = {
          'Epoch': epoch + 1,
