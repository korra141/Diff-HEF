...
#     return self.range_measurement
  ...
  def measurement(self):
    ...
    noise = np.random.normal(loc=0.0, scale=self.measurement_noise, size=3)
    measurement_ = self.position.parameters() + noise
    # measurement_[0:2] = (measurement_[0:2] + 0.5) % 1.0 - 0.5
    measurement_[0:2] = np.sign(measurement_[0:2]) * np.minimum(abs(measurement_[0:2]), (1 - abs(measurement_[0:2])))
...
MEASUREMENT_NOISE = np.array([0.1, 0.1, 0.1])
...
class ConvDensityEstimator(nn.Module):
    def __init__(self, input_shape=(50, 50, 32)):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(in_channels=2, out_channels=4, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv3d(in_channels=4, out_channels=2, kernel_size=3, padding=1),
            nn.ReLU(),
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # print(f"Time for forward pass: {time.time() - start_time_forward:.2f}s")
        ...
        loss = nll + scaling_factor * smoothness_loss
        # Backward pass
        optimizer.zero_grad()
        loss.backward()
        # print(f"Time for backward pass: {time.time() - start_time_backward:.2f}s")
        # total_norm = 0.0
        ...
        true_distribution_se2 = GaussianDistribution_se2(inputs.unsqueeze(1), cov,grid_size)
        energy_se2 = true_distribution_se2.energy(poses)
        energy_se2 = energy_se2.reshape((batch_size, *grid_size))
        nll_se2 = fft.neg_log_likelihood(energy_se2, targets)
        nll_se2_gaussian = true_distribution_se2.negative_log_likelihood(targets.unsqueeze(1)).reshape((batch_size, -1))
        ...
        if epoch % 10 == 0 and batch_idx == sample_batch:
            ...
            for j in indices:
                ...
                plot_dict = {
                    ...
                    'predicted_density': torch.sum(output_density[j],dim=2) * (2*math.pi/grid_size[2]),
                    }
                ...
                plot_distributions_s1(MEASUREMENT_NOISE[2], grid_size[2], torch.log(predicted_density + 1e-8), inputs[:,2] + math.pi, targets[:,2] + math.pi, epoch, batch_idx, j, logging_path)

    epoch_loss = running_loss / len(train_loader)
    logging.info(f"Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss:.4f}, Estimated NLL: {nll_tot/ len(train_loader):.4f}, Smoothening Loss: {smoothness_tot/len(train_loader):.4f}, True NLL: {true_nll_input_tot/len(train_loader):.4f}, True NLL Gaussian: {true_nll_tot_gaussian/len(train_loader):.4f} Time: {time.time() - start_time:.2f}s")
