...
def diff_hef_step(prior_pdf, inputs, measurements, control, poses, X, Y, T, grid_size, hef_filter, model, fft, hed_r1, optimizer, MOTION_NOISE, batch_size, regularizer_weight):
    with torch.no_grad():
    ...
    nll_measurement_likelihood = hed_r1.negative_log_likelihood(predicted_measurement_density_flat, measurements)
    # Calculate the regularizer weight
    ...
    # Combine the losses with the regularizer
    loss = (1 - regularizer_weight) * nll_measurement_likelihood + regularizer_weight * nll_posterior
    # Backpropagation and optimization
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()

    return posterior_pdf, predicted_pose, loss, nll_measurement_likelihood, nll_posterior
...
def main(args):
    ...
    MOTION_NOISE = np.sqrt(np.array(args.motion_noise))
    MEASUREMENT_NOISE = np.sqrt(args.measurement_noise)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        regularizer_weight = epoch / NUM_EPOCHS
        for j, data in enumerate(train_loader):
            ...
            for i in range(TRAJECTORY_LENGTH):
                ...
                posterior_pdf_diff, predicted_pose, loss, nll_likelihood, nll_posterior = diff_hef_step(prior_pdf_diff, inputs[:, i], measurements[:, i], control[:, i], poses, X, Y, T, grid_size, diff_hef_filter, model, diff_fft, hed_r1, optimizer, MOTION_NOISE, batch_size,regularizer_weight)
                ...
                with torch.no_grad():
                    ...
                    nll_likelihood_tot += float(nll_likelihood.item())
                    nll_posterior_tot += float(nll_posterior.item())
                    nll_posterior_true_tot += float(nll_posterior_true.item())
                
                # print(f"Time taken for step: {datetime.datetime.now() - start_time_step}")
        ...
        table_data = [
            ...
            ["Loss", loss_tot / (TRAJECTORY_LENGTH * len(train_loader))],
            ["Diff HEF NLL Likelihood", nll_likelihood_tot / (TRAJECTORY_LENGTH * len(train_loader))],
            ["Diff HEF NLL Posterior", nll_posterior_tot / (TRAJECTORY_LENGTH * len(train_loader))],
        ...
        wandb.log({ "Epoch": epoch, "Loss": loss_tot / (TRAJECTORY_LENGTH * len(train_loader)),"Diff HEF NLL Likelihood": nll_likelihood_tot/(TRAJECTORY_LENGTH * len(train_loader)),  "Diff HEF NLL Posterior": nll_posterior_tot / (TRAJECTORY_LENGTH * len(train_loader)), "True HEF NLL Posterior": nll_posterior_true_tot / (TRAJECTORY_LENGTH * len(train_loader)), "Diff HEF RMSE": mean_rmse_tot / len(train_loader), "True HEF RMSE": mean_rmse_true_tot / len(train_loader) })
        ...
        # Save the model
        if (epoch + 1) % 50 == 0:
            model_save_path = os.path.join(logging_path, f"measurement_model_epoch_{epoch + 1}.pth")
...
def parse_args():
    ...
    parser.add_argument('--trajectory_length', type=int, default=10, help='Length of each trajectory')
    parser.add_argument('--step_motion', type=float, nargs=3, default=[0.01, 0.00, np.pi / 40], help='Step motion parameters')
    parser.add_argument('--motion_noise', type=float, nargs=3, default=[0.1, 0.1, 0.04], help='Motion noise parameters')
    parser.add_argument('--measurement_noise', type=float, default=0.5, help='Measurement noise')
    parser.add_argument('--batch_size', type=int, default=40, help='Batch size')
    parser.add_argument('--validation_split', type=float, default=0.2, help='Validation split')
    parser.add_argument('--grid_size', type=int, nargs=3, default=[32, 32, 16], help='Grid size')
    parser.add_argument('--cov_prior', type=float, nargs=3, default=[0.05, 0.05, 0.01], help='Covariance prior')
