...
class MeasurementModel(nn.Module):
    def __init__(self, grid_size=(25, 25, 32)):
        ...
        self.conv1 = nn.Conv3d(in_channels=2, out_channels=16, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, p_x, z):
        ...
        return x.view(x.size(0), *self.grid_size)
...
def diff_hef_step(motion_inv_cov, inputs, measurements, range_beacon, control, poses, X, Y, T, grid_size, hef_filter, model, fft, hed_r1, motion_cov, batch_size):
    ...
    _, _, _, _, _, measurement_model.eta = measurement_model.fft.analyze(energy)
    measurement_model.energy, _, _, _, _, _ = measurement_model.fft.synthesize(measurement_model.eta)
        ...
        for data in val_loader:
            ...
            for i in range(args.trajectory_length - 1):
                ...
                posterior, measurement_model, belief_hat, predicted_pose, nll_measurement_likelihood, nll_posterior = diff_hef_step(
                    motion_inv_cov, inputs[:, traj_idx], measurements[:, i], range_beacon, control[:, i], poses, X, Y, T, args.grid_size, diff_hef_filter, model, diff_fft, hed_r1, motion_cov, args.batch_size
                )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for j, data in enumerate(train_loader):
            ...
            for i in range(TRAJECTORY_LENGTH - 1):
                ...
                range_beacon = beacons[beacon_id[:, i], :]
                posterior, measurement_model, belief_hat, predicted_pose, nll_measurement_likelihood, nll_posterior = diff_hef_step(motion_inv_cov, inputs[:, traj_idx], measurements[:, i],range_beacon, control[:, i], poses, X, Y, T, grid_size, diff_hef_filter, model, diff_fft, hed_r1, motion_cov, batch_size)
...
# def profile_training(args,loggin_path):
...
#     # stats.strip_dirs()
#     profiler.print_stats(sort='cumulative')  # Show top 10 time-consuming functions

    
def parse_args():
    parser = argparse.ArgumentParser(description="Diff-HEF SE2 Range Simulator")
    parser.add_argument('--num_epochs', type=int, default=200, help='Number of epochs')
    parser.add_argument('--num_trajectories', type=int, default=100, help='Number of trajectories')
