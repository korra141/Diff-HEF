...
# local import 
from src.utils.visualisation import plot_3d_density,generate_gif,plot_3d_analytic_filter
...
def main(args):
    ...
    for batch_idx, (ground_truth, measurements) in enumerate(train_loader):
        ...
        for j in range(trajectory_length):      
            ...
            if batch_idx == 0 and epoch % 20 == 0:
              ...
              dict_density = {
                'true_density': true_density[sample_idx],
                'measurement_pdf': measurement_pdf[sample_idx],
                'prediction_pdf': density_bel_x_t_bar[sample_idx],
                'posterior_pdf': posterior_pdf[sample_idx],
                'input_measurement_pdf': input_measurement_pdf[sample_idx],
              }
              plot_3d_density(ground_truth[sample_idx, j], measurements[sample_idx, j], range_x, range_y, band_limit, epoch_dir, dict_density, f"Epoch{epoch}_Iteration{j}", j)
          
              dict_density_analytic = {
              'analytic_posterior': analytic_posterior[sample_idx],
              'analytic_measurement': analytic_measurement[sample_idx],
              'analytic_predict': analytic_predict[sample_idx]
              }
              plot_3d_analytic_filter(ground_truth[sample_idx, j], measurements[sample_idx, j], range_x, range_y, band_limit, epoch_dir, dict_density_analytic, f"Epoch{epoch}_Iteration{j}_Analytic", j)
            prior_pdf = posterior_pdf 
  ...
  for epoch_folder in os.listdir(log_dir):
    ...
    if os.path.isdir(epoch_folder_path):
      video_path_1 = str(epoch_folder_path + f"/video_{epoch_folder}_plot3d.gif")
      generate_gif(epoch_folder_path, video_path_1, prefix='plot3d', duration=2)
      wandb.log({rf"video_{epoch_folder}_plot3d": wandb.Video(video_path_1)})

      video_path_2 = str(epoch_folder_path + f"/video_{epoch_folder}_2d_plots.gif")
      generate_gif(epoch_folder_path, video_path_2, prefix='2d_plots', duration=2)
      wandb.log({rf"video_{epoch_folder}_2d_plots": wandb.Video(video_path_2)})

      video_path_3 = str(epoch_folder_path + f"/video_{epoch_folder}_hef_2d_plots.gif")
      generate_gif(epoch_folder_path, video_path_3, prefix='hef_2d_plots', duration=2)
      wandb.log({rf"video_{epoch_folder}_hef_2d_plots": wandb.Video(video_path_3)})
