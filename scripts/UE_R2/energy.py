...
# local import 
from src.utils.visualisation import plot_3d_density
...
def main(args):
  ...
  for epoch in range(num_epochs):
    ...
    for batch_idx, (ground_truth, measurements) in enumerate(train_loader):
      ...
      input_pose_energy = input_pose_pdf.energy_over_grid()
      input_pose_density = input_pose_pdf.density_over_grid()
      outputs = model(input_pose_energy)

      check_tensor_nan(outputs)
      ln_z = hed.normalization_constant_energy(outputs)
      check_tensor_nan(ln_z)
      predicted_energy = outputs - ln_z

    #   predicted_density = hed.convolve(input_pose_density,outputs)
      hef_loss_input = hed.loss_energy(input_pose_energy,measurements)
      initial_nll = input_pose_pdf.negative_log_likelihood(measurements)
    
      loss = hed.loss_energy(predicted_energy, measurements)
      ...
      true_energy = target_distribution.energy_over_grid()

    #   mode = hed.mode(predicted_energy)
    #   rmse_tot += root_mean_square_error(mode,ground_truth).item()
    #   mae_tot += mean_absolute_error(mode,ground_truth).item()
      predicted_density = torch.exp(predicted_energy)
    ...
    #       wandb.log({'batch': batch_step,f"gradients/{name}": wandb.Histogram(param.grad.cpu().numpy())})
      ...
      print(f"Batch: {batch_idx}, hef_input: {hef_loss_input.item()}, initial_nll: {initial_nll.item()}, loss: {loss.item()}")
      ...
      if batch_idx==0 and epoch % 5 == 0:
        ...
        plot_dict = {
          ...
          'predicted_energy': predicted_density[sample_idx],
          'input_pose_energy': input_pose_density[sample_idx]
        }
        plot_3d_density(ground_truth[sample_idx],measurements[sample_idx],range_x,range_y,band_limit,epoch_dir,plot_dict,"energy")
...
if __name__ == "__main__":
  ...
  parser.add_argument('--band_limit', type=str, default="20 20",help="Bandlimit values as a string")
