...
def multimodal_gaussian_noise(args):
  # variance1 = heteroscedastic_noise(x, args.measurement_noise_min, args.measurement_noise_min)
  # variance2 = heteroscedastic_noise(x, args.measurement_noise_min, args.measurement_noise_min)
  std1 = args.measurement_noise_min
  std2 = args.measurement_noise_max
  mean1 = -args.mean_offset / 2
  mean2 = +args.mean_offset / 2
  noise1 = np.random.normal(mean1, std1, 1)
  noise2 = np.random.normal(mean2, std2, 1)
...
def generating_data_S1(args, base_path, batch_size, n_samples, trajectory_length, step=0.1, shuffle_flag=True):
  ...
  if not os.path.exists(data_path):
    ...
    for i in range(n_samples):
      ...
      noise = np.squeeze(np.array([multimodal_gaussian_noise(args) for x in trajectory]))
      
      measurements[i] = (trajectory + noise + 2*np.pi) % (2 * np.pi)
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (ground_truth, measurements) in enumerate(train_loader):
            ...
            print(f'Epoch [{epoch+1}/{args.num_epochs}], Batch [{i+1}/{len(train_loader)}], Loss: {loss.item():.4f}, time: {time.time() - start_time:.4f}')
