...
def generate_bounded_se2_dataset(
    ...
    measurements = np.ndarray((num_trajectories, trajectory_length,80000))
    ...
    dataset = torch.utils.data.TensorDataset(ground_truth_torch, measurements_torch, control_torch)
    ...
    train_loader = torch.utils.data.DataLoader(dataset, batch_size=1, drop_last=True, shuffle=False,  pin_memory=True)
