...
class HarmonicExponentialDistribution:
    ...
    def negative_log_likelihood(self,energy, measurements, range_L):
        ...
        exponential_term = torch.exp(1j * (2 * math.pi / range_L) * k_values * value)  # [batch_size, num_samples, 1]
    ...
    def normalization_constant(self,energy):
        maximum = torch.max(energy.real, dim=-1).values.unsqueeze(-1)
