...
class BetaDistribution:
    ...
    def density_over_grid(self,sample_size,batch_size):
        ...
        return torch.tensor(self.pdf(grid)).type(torch.FloatTensor)
    ...
    def density_translated(self, theta_mean,sample_size):
        ...
        normalise_mean = theta_mean/(2*math.pi)
        density_over_grid = self.density_over_grid(sample_size,theta_mean.shape[0])
        batch_size, seq_len = density_over_grid.shape
        shift_indices = (torch.arange(seq_len).unsqueeze(0) - (normalise_mean * sample_size).to(torch.int)) % seq_len  # Wrap indices
    ...
    def negative_loglikelihood(self, x):
