...
#             T *= self.n_samples_t / (2 * torch.pi)
    ...
    def neg_log_likelihood(self, energy, pose: torch.Tensor) -> torch.Tensor:
        ...
        f = -torch.sum(f_p_p * torch.exp(angle), dim=(1, 2, 3)).real + l_n_z.to(energy.device)
    ...
    def compute_moments_lnz(self, energy):
        ...
        """
        maximum = torch.max(torch.abs(energy)).to(energy.device)
        _ , _, _, _, _, unnormalized_moments_ = self.analyze(torch.exp(energy - maximum))
        ...
        constant = (unnormalized_moments[:, z_0, z_1, z_2] * (math.pi * 2* 2* math.pi)).to(energy.device)
        l_n_z = torch.log(constant + 1e-8) + maximum
